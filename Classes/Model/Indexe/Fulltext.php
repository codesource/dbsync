...
/**
 * Table fulltext key model
 *
 * @author Matthias Toscanelli <m.toscanelli@code-source.ch>
 */
class Fulltext extends AbstractIndexe {
  ...
  public function drop($type) {
    switch ($type) {
      case Type::MYSQL:
        return 'ALTER TABLE `' . $this->table->getName() . '` DROP INDEX ' . $this->name;
    }
    return '';
  }

  protected function addMysql() {
    $result = 'CREATE FULLTEXT INDEX '  . $this->name;
    if ($this->type === self::TYPE_HASH) {
      $result .= ' USING HASH';
    } elseif ($this->type === self::TYPE_BTREE) {
      $result .= ' USING BTREE';
    }
    $result .= ' ON `' . $this->table->getName() . '`';
    $columns = array();
